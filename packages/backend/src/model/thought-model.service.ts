...
import { LLMCanUse, StreamingChunk } from '@not_stone/shared';
import { z } from 'zod';
...
export class ThoughtModelService {
	...
	async getGeminiThinkingModelDivided(
		config: ChatOpenAIFields | ChatGoogleGenerativeAI | LLMCanUse
	...
	async getGeminiThinkingModelFlat(
		...
		switch (config) {
			case LLMCanUse.gemini_2_5_pro_proxy:
				llm = this.modelService.getLLMGeminiRaw('gemini-2.5-pro', apiKey); //代理的apiKey
				if (schema) {
					llm = llm.withStructuredOutput(schema) as any;
				}
				break;
			case LLMCanUse.gemini_2_5_pro:
				llm = this.modelService.getLLMGeminiPlusRaw('gemini-2.5-pro', apiKey);
				if (schema) {
					llm = llm.withStructuredOutput(schema) as any;
				}
				break;
			case LLMCanUse.gemini_2_5_flash:
				llm = this.modelService.getLLMGeminiPlusRaw('gemini-2.5-flash', apiKey);
				if (schema) {
					llm = llm.withStructuredOutput(schema) as any;
				}
				break;
	 ...
	 * @returns
	 */
	async getGLMThinkingModelFlat(config: LLMCanUse, apiKey: string, schema?: z.Schema) {
		let llm: ChatOpenAI;
		switch (config) {

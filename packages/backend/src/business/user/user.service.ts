...
import { ErrorCode, RegistResponse } from '@not_stone/shared';
...
export class UserService {
	...
	async regist(user: RegisterUserDto, jumpCapcha = false) {
		const captcha = await this.redisService.get(`captcha_${user.email}`);

		if (!captcha && !jumpCapcha) {
			throw new Error(ErrorCode.CAPTCHAEXPIRED);
		}

		if (user.captcha !== captcha && !jumpCapcha) {
